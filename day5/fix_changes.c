...
int validate(int u, int *update) {
    int block[TABLE_SIZE] = {0};
    for (int i=0; i<u; i++) {
        blockPages(block, update[i]);
        if (block[update[i]] != 0) return i;
    }
    return 0;
}
...
int main() {
    ...
    while (fgets(line, sizeof(line), stream) != NULL) {
        if (lc < SECTION_BREAK) {
            ...
            if (validate(u, update) == 0) continue;
            int bi;
            while ((bi = validate(u, update)) != 0) {
                int tmp = update[bi];
                update[bi] = update[bi-1];
                update[bi-1] = tmp;
            }
            sum += update[u/2];
        }
    ...
    printf("Sum of middle pages from fixed updates: %d\n", sum);
